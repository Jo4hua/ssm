...
<fieldset style="height:490px;">
		<legend><a href="index.jsp">首页</a>>><a href="SearchUser?num=1&name=">用户管理界面</a></legend>
		<form method="post" action="SearchUser?num=1">
			...
			<div style="width:100%;height:260px;" align="center"><!--数据  -->
				...
				<table style="width:80%;text-align:center" border="1" cellspacing="0" align="center">
					<tr>
						...
						<td>
							<font color="grey">*管理员只能对普通用户进行操作</font>
						</td>
					</tr>
				<%for(Object o:list)
				{
			...
			<a href="SearchUser?num=1&name=<%=request.getParameter("name") %>">首页</a>
			<a href="SearchUser?num=<%=p.getPre() %>&name=<%=request.getParameter("name") %>">上一页</a>
			<%for(int i=p.getsPage();i<=p.getePage();i++){ %>
				<a href="SearchUser?num=<%=i %>&id="><%=i %></a>
			<%} %>
			<a href="SearchUser?num=<%=p.getNext() %>&name=<%=request.getParameter("name") %>">下一页</a>
			<a href="SearchUser?num=<%=p.getSum() %>&name=<%=request.getParameter("name") %>">尾页</a>
			</div>
</fieldset>
</div>
